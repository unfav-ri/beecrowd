...
SELECT
    ...
    CASE
        WHEN COALESCE(SUM(vencimento.valor), 0) = 0 THEN 0
        ELSE COALESCE(SUM(vencimento.valor), 0)
    END AS "Salario Bruto",
    CASE
        WHEN COALESCE(tabela.val_desc, 0) = 0 THEN 0
        ELSE COALESCE(tabela.val_desc, 0)
    END AS "Total Desconto",
    COALESCE(SUM(vencimento.valor), 0) - COALESCE(tabela.val_desc, 0) as "Salario Liquido"
FROM
    emp_venc
